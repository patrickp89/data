...
Next we used these variables in a 10-fold cross-validated linear regression model.
```{r linReg1, echo=FALSE}
ctrl <- trainControl(method = "cv",
                     number = 10)

linreg1 <- train(Order_Line_Amount ~ Order_Line_Day_of_Week + Gender + US_State,
                 data = orderDfCleaned,
                 trControl = ctrl,
                 na.action  = na.pass,
                 method = "lm")
...
```{r prediction2}
unseenCustomer2 <- data.frame(Order_Line_Day_of_Week = c("Thursday"),
                             Gender = c("Male"),
                             Age = c(23),
                             US_State = c("CO"))
predict(linreg1, unseenCustomer2, se.fit = TRUE)
```
