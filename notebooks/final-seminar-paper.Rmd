...
GDP_statistics <- read.csv(file="../data/external/GDP_per_state_2000.csv", sep=",", quote="\"")
...
clickstream_data <- cleanFile("../data/interim/clickstream/clickstream_with_headers.csv", 0.85, c("US_State"))
orderDfCleaned <- cleanFile("../data/interim/orders/orders_with_headers.csv", 0.85, c())

GDP_statistics <- GDP_statistics %>%
  transmute(GeoFips = sapply(strsplit(as.character(GeoFips..GeoName...2000.), ","), "[", 1),
         State = sapply(strsplit(as.character(GeoFips..GeoName...2000.), ","), "[", 2),
         GDP = as.numeric(sapply(strsplit(as.character(GeoFips..GeoName...2000.), ","), "[", 3))) %>%
...
GDP_statistics[order(GDP_statistics$GDP, decreasing = TRUE),] %>%
  select(State, GDP) %>%
